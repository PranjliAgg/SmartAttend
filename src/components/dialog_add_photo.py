...
def add_photos_dialog():
    ...
    with t1:
        ...
        if st.button("Camera", type=type_camera, width="stretch", key="camera", on_click=set_camera_tab):
            st.session_state.photo_tab = "camera"

    with t2:
        type_upload = "primary" if st.session_state.photo_tab == "upload" else "secondary"
        if st.button("Upload Photos", type=type_upload, width="stretch", key="upload", on_click=set_upload_tab):
            st.session_state.photo_tab = "upload"
    ...
    if st.button("Done", type="tertiary", width="stretch"):
        del st.session_state.photo_tab
        st.rerun()